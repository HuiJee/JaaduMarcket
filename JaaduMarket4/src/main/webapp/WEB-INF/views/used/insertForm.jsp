...
<form method="POST" enctype="multipart/form-data" action="${cpath}/used/insert" onsubmit="return check(this)">
	<section class="category">
		...
		<option value="11">---카테고리 선택---</option>
			<c:forEach var="vo" items="${clist }">
				<option value="${vo.u_cate_seq }">${vo.u_cate_name }</option>
			</c:forEach>
		</select> 
		<input type="text" id="textInput" style="display: none;" placeholder="브랜드 등 추가사항(선택)" width="auto" name="cate_input">
		
		<div id="subSelect" style="display: none;">
			<select name="cate_check_name">
				<option value="" selected>---선택하세요---</option>
				<c:forEach var="sub" items="${subList }">
					<option value="${sub.cate_check_name }" style="display: none;" name="${sub.u_cate_seq}">${sub.cate_check_name }</option>
				</c:forEach>
			</select>
		</div>
	</section>
	...
	<section class="form">
		...
		<div>
			...
			<hr>
			<div class="price">
				<input type="text" name="u_price" id="sharePrice">원
				<h4 style="padding-left:50px;">나눔 여부</h4>(나눔의 가격은 0원입니다.)&nbsp;
				<input type="checkbox" name="u_share" value="1" onclick="shareThis()" id="share">
			</div>
		</div>
		<div>
			<h2>설명</h2>
			<hr>
			<textarea id="content" name="u_content" cols="100" rows="30"></textarea></td>
		</div>
		<div>
	...
	<button type="submit" style="margin:30px 0px;">등록하기</button>
</form>
